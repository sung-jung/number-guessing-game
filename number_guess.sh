...
# variable to query database quietly
...
# fetch USER_ID for both new and returning users (trim whitespace)
USER_ID=$($PSQL "SELECT user_id FROM users WHERE username='$USERNAME'" | tr -d ' ')

# if returning user, get games played and best game
if [[ $USERNAME_RESULT ]]; then
